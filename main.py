#!/usr/bin/env python3
"""Main entry point for Huispedia scraper."""
...
from scraper import HuispediaScraper
from models import Property
from config import LOCATIONS, PROPERTY_TYPES, DEFAULT_MAX_WORKERS
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Scrape property listings from Huispedia.nl",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s --location amsterdam
  %(prog)s --location rotterdam --property apartment
  %(prog)s --location utrecht --max-pages 5
  %(prog)s --location den-haag --limit 100
  %(prog)s --location eindhoven --max-pages 3 --no-details
  %(prog)s --location amsterdam --max-pages 2 -v
        """,
    )
    ...
    parser.add_argument(
        "--location", "-l",
        default="amsterdam",
        help=f"Location to search. Available: {', '.join(LOCATIONS.keys())} (default: amsterdam)",
    )
    parser.add_argument(
        "--property", "-p",
